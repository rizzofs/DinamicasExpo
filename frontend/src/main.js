      ...
      <div class="animate-fadeIn" style="width: 100%; height: 100%; display: flex; flex-direction: column; align-items: center; justify-content: center;">
        ...
        <div id="ia-visualizer" style="width: 100%; max-width: 800px; padding: 2rem; margin-top: 2rem;" class="glass-panel">
          <div style="display: flex; justify-content: space-between; margin-bottom: 1rem;">
            <span>Preguntas Respondidas: <span id="ia-data-count">0</span>/10</span>
          ...
          <div id="neural-net-view" style="height: 350px; margin-top: 2rem; display: flex; align-items: center; justify-content: center; position: relative; background: url('/cerebro_ia.png') no-repeat center; background-size: contain;">
             <canvas id="ia-canvas" style="width: 100%; height: 100%; display: block;"></canvas>
             <div id="ia-result-overlay" style="display: none; position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: rgba(9, 10, 15, 0.9); flex-direction: column; justify-content: center; align-items: center; border-radius: 10px;">
               <h3 class="text-gradient-ia" style="font-size: 2.5rem;">¡IA Optimizada!</h3>
               <h2 id="ia-profile-name" style="font-size: 3.5rem; margin-top: 1rem; color: #00f2fe; text-shadow: 0 0 20px rgba(0,242,254,0.6);">SÚPER TECH</h2>
               <p style="color: var(--color-text-muted); margin-top: 0.5rem; font-size: 1.2rem;">¡Tenés un potencial enorme en tecnología!</p>
             </div>
          </div>
        </div>
      </div>
...
function updateMobileConceptUI(conceptId) {
  ...
  if (conceptId === 1) {
      ...
      <div style="flex: 1; display: flex; flex-direction: column; justify-content: space-between;">
        <div>
          ...
          <div id="tinder-card" class="glass-panel" style="width: 100%; aspect-ratio: 1/1; display: flex; flex-direction: column; justify-content: center; align-items: center; padding: 2rem; border-radius: 20px; position: relative;">
             ...
             <h4 style="margin-top: 1rem;">¿Es Inteligencia Artificial?</h4>
          </div>
        </div>

        <div style="display: flex; gap: 1rem; margin-top: 1rem;">
          <button id="swipe-left" class="btn-premium" style="flex: 1; background: rgba(255,0,0,0.2); border: 1px solid red; color: white;">❌ NO</button>
          <button id="swipe-right" class="btn-premium" style="flex: 1; background: rgba(0,255,0,0.2); border: 1px solid green; color: white;">✔ SÍ</button>
        </div>
      </div>
    `
    ...
    const cardTexts = [
      '¿Los filtros de TikTok usan IA?',
      '¿Los NPC de los juegos piensan solos?',
      '¿Spotify sabe qué te gusta usando IA?',
      '¿Tu celular puede clonar tu voz?',
      '¿ChatGPT razona igual que un humano?',
      '¿Los Deepfakes se hacen con IA?',
      '¿Una IA puede sentir emociones?',
      '¿El auto-corrector predice usando IA?',
      '¿Se puede usar la IA para ciberataques?',
      '¿Las imágenes de Midjourney usan IA?'
    ]

    const handleSwipe = (direction) => {
      swipeCount++
      socket.emit('swipe_data', { direction, count: swipeCount })
      
      if (swipeCount >= 10) {
        socket.emit('training_complete', { profile: 'Data Scientist' })
        container.innerHTML = `<div style="text-align:center; padding-top: 3rem;">
          <h3 class="text-gradient-ia">¡IA Entrenada!</h3>
          <p style="margin-top:1rem;">Mirá el resultado en la pantalla.</p>
        </div>`
      } else {
        cardEl.querySelector('span').textContent = cardIcons[swipeCount % cardIcons.length]
        cardEl.querySelector('h4').textContent = cardTexts[swipeCount % cardTexts.length]
      }
    }
    ...
    container.innerHTML = `
      <div style="flex: 1; display: flex; flex-direction: column; justify-content: center; align-items: center; gap: 1rem;">
        ...
        <p style="color: var(--color-text-muted); font-size: 0.8rem; margin-top: 1rem; text-align: center;">¡Llegá hasta la nuez 🌰 para ganar!</p>
      </div>
    `
    const sendMove = (dir) => {
...
function initConcept2TV() {
  ...
  function animateBrain() {
    ...
    // Dibujar ráfagas de luz (sinapsis neuronales)
    bursts.forEach((b, index) => {
      ctx.lineWidth = 3
      ctx.strokeStyle = `rgba(0, 242, 254, ${b.alpha})`
      ctx.shadowBlur = 15
  ...
  socket.on('process_swipe', (data) => {
    ...
    if (countEl && progressEl) {
      ...
      progressEl.style.width = `${(data.count / 10) * 100}%`
    }

    // Generar ráfaga eléctrica en una posición aleatoria del cerebro
    bursts.push({
